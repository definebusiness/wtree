...
|---|---|---|
| P2 / §7.1 release locks | delivered under the focused release-lock plan through strict local generation, Git-authenticated exact CI materialization, local `post-release` hooks, and the executable source-to-CI tutorial; the plan remains nonterminal while its final documentation milestone is independently reviewed and verified. A separate verification command, lock-aware update, and general status/doctor integration remain deferred | [Immutable release lock manifests specification](release-lock-manifests.md) and [implementation plan](../plans/release-lock-manifests.md), with strict schema, selected revisions, authenticated acquisition, publication, rollback, recovery, compatibility, full documentation, and an executable release tutorial |
...
| P3 / §8 portable `post-clone` hooks | explicit portable trust, execution, retries, and hook recovery; local/shared hooks remain focused follow-on work | focused hook-trust and retry specification and plan; no manifest code execution before acquisition commits and no shared-hook execution from the manifest |
