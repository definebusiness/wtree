...
| [Local and shared workspace lifecycle hooks implementation plan](plans/local-workspace-lifecycle-hooks.md) | `initial` | [Local and shared workspace lifecycle hooks specification](spec/local-workspace-lifecycle-hooks.md) | M00–M05 are independently approved and verified. M06 is locally implemented, independently approved, and verified: its [executable lifecycle-hook tutorial](../tutorial/LIFECYCLE-HOOKS.md), decoded v2/v3 public-contract matrix, public/installed documentation, privacy and process traceability, retained full normal/race suites, and all local quality gates pass. Exact-tree commit/push/PR publication is now authorized and the matching Ubuntu/macOS/Windows Actions evidence is pending; M06 and the plan remain unchecked until that hosted evidence and final verification complete. The active [durable run](ai/runs/local-workspace-lifecycle-hooks.md) records the authoritative state, and the focused [implementation context](plans/local-workspace-lifecycle-hooks-context.md) captures integration seams and hazards |
