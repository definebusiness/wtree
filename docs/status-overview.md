...
| [Windows portability and CI hardening implementation plan](plans/windows-portability-and-ci-hardening.md) | `initial` | [Windows portability and CI hardening specification](spec/windows-portability-and-ci-hardening.md) | Authorized [durable run](ai/runs/windows-portability-and-ci-hardening.md) has M00–M02 independently approved and verified. On the dedicated `fix/windows-portability-simplification` branch, exact N2 run [`33227208739`](https://github.com/definebusiness/wtree/actions/runs/33227208739) passed every Ubuntu gate in 7m21s and every macOS gate in 22m46s. Windows passed checkout, formatting, vet, and completed all eight normal service shards; its final two top-level failures shared one test-fixture root that selected the Windows private container instead of its logical clone child. The three-test-file N3 correction is independently approved with focused normal/race, all eight 538/538 service shards, full non-service and Git suites, Windows service cross-compilation, helper, vet, formatting, and diff gates green. Attempts remain 2. M03 awaits a matching hosted rerun of the approved N3 tree. The focused [simplification and CI remediation plan](plans/windows-portability-simplification-and-ci-remediation.md) and its [hosted failure context](plans/windows-portability-simplification-and-ci-remediation-context.md) remain supporting design records, not a separately active run. |
