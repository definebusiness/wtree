...
| [Windows portability and CI hardening implementation plan](plans/windows-portability-and-ci-hardening.md) | `initial` | [Windows portability and CI hardening specification](spec/windows-portability-and-ci-hardening.md) | Authorized [durable run](ai/runs/windows-portability-and-ci-hardening.md) has M00–M02 independently approved and verified. On the dedicated `fix/windows-portability-simplification` branch, exact run [`33223574628`](https://github.com/definebusiness/wtree/actions/runs/33223574628) passed every Ubuntu and macOS gate; Windows passed format and vet, then all normal partitions completed and reduced the native failures from N1's 45 top-level tests to seven clone/update tests before race/build/release. The exhaustive N2 repair is independently approved; exact focused normal/race, all eight 538/538 service shards, full non-service packages, Windows service cross-compilation, the deterministic helper, vet, formatting, and diff gates are green. M03 remains unchecked until the authorized N2 commit/push produces a matching fully green hosted matrix. The focused [simplification and CI remediation plan](plans/windows-portability-simplification-and-ci-remediation.md) and its [hosted failure context](plans/windows-portability-simplification-and-ci-remediation-context.md) remain supporting design records, not a separately active run. |
