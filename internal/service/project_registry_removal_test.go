...
func TestProjectPruneLockedRacesFailBeforeWriter(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "eligibility changed", mutate: func(t *testing.T, data string) {
			...
			if err := config.WriteProjectFile(path, config.ProjectConfig{Version: config.Version, Project: config.Project{ID: "stale", Name: "stale"}, Repositories: map[string]config.Repository{"root": {Source: ".", DefaultMount: "."}}}); err != nil {
				t.Fatal(err)
			}
		}},
